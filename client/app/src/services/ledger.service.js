;(function () {
  ...
  function LedgerService ($q, $http, $timeout, storageService, networkService) {
    ...
    const makejs = require(_path.resolve(__dirname, '../node_modules/makejs'))
    ...
    function getBip44Accounts (slip44) {
      ...
      async.whilst(
        ...
        (next) => {
          ...
          if (result.address) {
            result.address = makejs.crypto.getAddress(result.publicKey)
    ...
    function recoverBip44Accounts (backupLedgerPassphrase, slip44) {
      const hdnode = makejs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase))
    ...
    function signTransaction (path, transaction) {
      return new Promise((resolve, reject) => {
        ...
        ipcRenderer.send('ledger', {
          ...
          data: makejs.crypto.getBytes(transaction, true, true).toString('hex'),
          path
        })
      })
    }
